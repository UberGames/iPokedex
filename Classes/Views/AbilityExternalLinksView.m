...
@interface AbilityExternalLinksView (hidden)
...
@implementation AbilityExternalLinksView
...
- (NSURL *)englishURLForLinkWithIndex:(NSInteger)index withGen:(NSInteger)gen
{
    ...
    switch (index )
    {
        case LINK_EN_BULBAPEDIA:
        {
            NSString *name = [abilityName stringByReplacingOccurrencesOfString: @" " withString: @"_"];
            address = [NSString stringWithFormat: @"http://bulbapedia.bulbagarden.net/wiki/%@_(ability)", name];
        }
            break;
        case LINK_EN_VEEKUN:
            address = [NSString stringWithFormat: @"http://veekun.com/dex/abilities/%@", abilityName];
            address = [address stringByReplacingOccurrencesOfString: @" " withString: @"%20"];
            break;
        case LINK_EN_SEREBII:
        {
            NSString *formattedName = [[abilityName stringByReplacingOccurrencesOfString: @" " withString: @""] lowercaseString];
            address = [NSString stringWithFormat: @"http://serebii.net/abilitydex/%@.shtml", formattedName];
        }
            break;
        case LINK_EN_SMOGON:
        {
            ...
            address = [NSString stringWithFormat: @"http://www.smogon.com/%@/abilities/%@", particle, linkName];
        }
            break;
    }
